...
class TestFileImport(unittest.TestCase):
    ...
    def test_return_study_plan_correct_format(self):
        self.plan.add_course_to_plan("SAA3", on_cur=True)
        self.plan.add_course_to_plan("OP1", on_cur=True)
        self.plan.add_course_to_plan("FY5", on_cur=True)

        self.plan.add_course_to_plan("EGE1", "EGEN KURS 1", 5, on_cur=False)
