...
class TestPlan(unittest.TestCase):
    ...
    def test_cannot_add_illegal_subjects(self):
        self.plan.add_subject_to_me_plan("BIO", 3)
        self.assertEqual(self.plan.return_subject_in_me_plan(), [])

    def test_cannot_add_subjects_with_illegal_period(self):
        self.plan.add_subject_to_me_plan("A", 6)
        self.assertEqual(self.plan.return_subject_in_me_plan(), [])

    def test_validate_legal_plan(self):
        self.plan.add_subject_to_me_plan("A", 1)
        self.plan.add_subject_to_me_plan("EA", 1)
        self.plan.add_subject_to_me_plan("N", 2)
        self.plan.add_subject_to_me_plan("GE", 3)
        self.plan.add_subject_to_me_plan("FY", 3)

        self.assertFalse(self.validation.validate(self.plan))

    def test_validate_illegal_plan_structure(self):
        self.plan.add_subject_to_me_plan("A", 1)
        self.plan.add_subject_to_me_plan("EC", 1)
        self.plan.add_subject_to_me_plan("GE", 3)
        self.plan.add_subject_to_me_plan("FY", 3)
        self.plan.add_subject_to_me_plan("HI", 2)
    ...
    def test_validate_illegal_plan_timing(self):
        ...
        self.plan.add_subject_to_me_plan("EC", 1)
        self.plan.add_subject_to_me_plan("GE", 3)
        self.plan.add_subject_to_me_plan("YH", 3)
        self.plan.add_subject_to_me_plan("A", 2)
